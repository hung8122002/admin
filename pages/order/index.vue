  ...
  <div style="position: relative">
    ...
    <div class="product-center">
      ...
      <div class="filter" v-show="showFilter">
        ...
        <v-select
          density="compact"
          :items="['pending', 'accept', 'deny', 'transported', 'delivered']"
          v-model="status"
          label="Status:"
          variant="solo"
          hide-details
        ...
        <table v-show="productList.length">
          ...
          <tbody>
            ...
            >
              ...
              <td class="name">{{ item.info.address }}</td>
              ...
              <td>
                <v-icon
                  @click.stop="(showDelete = true), (deleteItem = item)"
                  icon="mdi-delete"
                ></v-icon>
              </td>
            </tr>
    ...
    >
      ...
      <hr />
      <p style="margin: 12px 0">This order will be deleted from the list</p>
    </the-detail>
    ...
    >
      <div>
        <h3>Order detail</h3>
        <h5 class="mb-3">Receiver's information:</h5>
        <div class="order-detail-center">
          <v-text-field
            v-model="detailItem.info.email"
            class="mb-1"
            density="compact"
            label="Email"
            readonly
          ...
          ></v-text-field>
          <v-text-field
            v-model="detailItem.info.address"
            class="mb-1"
            density="compact"
            label="Address"
            readonly
          ...
          ></v-text-field>
          <v-text-field
            v-model="detailItem.info.phonenumber"
            class="mb-1"
            density="compact"
            label="Phone Number"
            readonly
          ...
          ></v-text-field>
          <v-text-field
            v-model="detailItem.info.fullname"
            class="mb-1"
            density="compact"
            label="Full Name"
            readonly
        ...
        <div>
          <h5 class="my-3">Order list:</h5>
          <v-chip
            v-for="item in detailItem.order"
            @click="setDetailBook(item.id)"
            :key="item.id"
            class="mr-2"
            color="green"
            text-color="white"
...
export default {
  ...
  computed: {
    urlApiToTal() {
      var urlApi = api.getOrder() + `?status_like=${this.filter.status}`;
      if (this.filter.email) {
        urlApi = urlApi + `&info.email_like=${this.filter.email}`;
      }
      if (this.filter.phoneNumber) {
        urlApi = urlApi + `&info.phoneNumber_like=${this.filter.phoneNumber}`;
      }
      if (this.filter.address) {
        urlApi = urlApi + `&info.address_like=${this.filter.address}`;
      }
      if (this.searchTextFilter) {
        urlApi = urlApi + `&info.fullname_like=${this.searchTextFilter}`;
      }
      return urlApi;
    },
    urlApi() {
  ...
  methods: {
    async getData() {
      ...
      this.productList = await common.method(
        "get",
        api.getOrder() +
          `?_page=${this.curentPage}&_limit=${this.perPage}&status_like=pending` +
    ...
    async confirmDelete() {
      ...
      update = await common.method(
        "delete",
        api.getOrder() + `/${this.deleteItem.id}`
      );
      if (update) {
        this.isValid = true;
        this.isSuccess = true;
